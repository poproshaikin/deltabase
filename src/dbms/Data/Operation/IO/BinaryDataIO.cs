...
public class BinaryDataIO
{
    ...
    protected readonly BinaryDataSerializer _serializer;
    ...
    public async Task<PageRow> ReadRowAsync(TableScheme scheme)
    {
        ...
        bool[] nullBitmap = new bool[scheme.Columns.Length];
        for (int i = 0; i < nullBitmap.Length; i++)
        {
            nullBitmap[i] = ReadByte() is 1;
        }

        string?[] values = new string?[scheme.Columns.Length];
    ...
    public async Task<int> ReadIntAsync()
    {
        ...
        return _serializer.DeserializeAsInt(buffer);
    }

    public async Task<float> ReadFloatAsync()
    {
        byte[] buffer = await ReadBytesAsync(sizeof(float));
        return _serializer.DeserializeAsFloat(buffer);
    }
    ...
    public async Task<string> ReadStringAsync()
    {
        ...
        return _serializer.DeserializeAsString(buffer);
    }
    ...
    public async Task<char> ReadCharAsync()
    {
        int length = await ReadIntAsync();
        byte[] buffer = await ReadBytesAsync(length);
        return _serializer.DeserializeAsChar(buffer);
    }
    ...
    public async Task SeekToRowAsync(int rowId)
    {
        await SeekToRowAsync(rowId, _scheme ?? throw new DbEngineException(ErrorType.InternalServerError));
    }

    public async Task SeekToRowAsync(int rowId, TableScheme scheme)
    {
        await SeekToFirstRow();
    ...
    private async Task SeekToFirstRow()
    {
    ...
    // /// <summary>
    // /// Prepares a stream to inserting row data 
    // /// </summary>
    // /// <param name="emptyRowId">Id of row to be inserted</param>
    // /// <returns>Id of last filled row</returns>
    // /// <exception cref="NotImplementedException"></exception>
    // public int PrepareToRowInsert(int emptyRowId)
    // {
    //     
    // }
}