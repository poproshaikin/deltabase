...
public class DltDatabase
{
    ...
    public DltDatabase(string name, FileSystemManager fs)
    {
        Name = name;
        _fs = fs;
        
        _records = new RecordsManager(dbName: Name, _fs);
        _records.Init();

        _parser = new QueryParser();
        _validator = new QueryValidator(_records);
        _executor = new QueryExecutor(_records);
    }

    public byte[] ExecuteRequest(string rawQuery)
    {
        IParsedQuery parsedQuery = _parser.Parse(rawQuery);
        
        ValidationResult validationResult = _validator.Validate(parsedQuery);
        
        if (!validationResult.IsValid)
            return ConvertHelper.GetBytes(validationResult.Error);
        
        ExecutionResult executionResult = _executor.Execute(validationResult.Query);
